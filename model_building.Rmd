...
summary(efa_abuse_f1, fit.measures = TRUE)
...
summary(efa_abuse_f2, fit.measures = TRUE)

# Increasing factors doesn't fix the identification problem
...
efa_household_f1 <- 
  cfa(model = household_f1,
      ...
      estimator = "WLSMV",
      mimic = "Mplus",
      ordered = c("ksads_ptsd_raw_766_p", "famhx_ss_parent_alc_p_rec", "famhx_ss_parent_dg_p_rec", "famhx_ss_parent_dprs_p_rec", "famhx_ss_parent_ma_p_rec", "famhx_ss_parent_vs_p_rec", "famhx_ss_parent_trb_p_rec", "famhx_ss_parent_nrv_p_rec", "famhx_ss_parent_prf_p_rec", "famhx_ss_parent_hspd_p_rec", "famhx_ss_parent_scd_p_rec", "demo_fam_exp2_v2_rec", "demo_fam_exp3_v2_rec", "demo_fam_exp4_v2_rec", "demo_fam_exp5_v2_rec", "demo_fam_exp6_v2_rec", "demo_fam_exp7_v2_rec")
      )
summary(efa_household_f1, fit.measures = TRUE)
# NaNs producedError in eigen(A, symmetric = TRUE, only.values = TRUE) : 
#  infinite or missing values in 'x'

efa_household_f1_mlr <- 
  cfa(model = household_f1,
      data = MB_subsample1,
      rotation = "oblimin",
      estimator = "MLR",
...
summary(efa_household_f1_mlr, fit.measures = TRUE)
...
summary(efa_neglect_f1_mlr, fit.measures = TRUE)
# 1 factor ran but chi-sq is significant.
...
summary(efa_neglect_f2_mlr, fit.measures = TRUE)
# The variance-covariance matrix of the estimated parameters (vcov)
#    does not appear to be positive definite! The smallest eigenvalue
#    (= -1.004072e-04) is smaller than zero. This may be a symptom that
#    the model is not identified.
...
summary(efa_stress_f1, fit.measures = TRUE)

# NaNs producedError in chol.default(tcrossprod(HEAD)) : 
...
summary(efa_stress_f1_mlr, fit.measures = TRUE)
...
summary(efa_social_f1_mlr, fit.measures = TRUE)
# Ran fine but chi-sq NS
...
summary(efa_social_f2_mlr, fit.measures = TRUE)
```
